...
function buildCoachPrompt(userContext: string, locale: AILocale): string {
  ...
  return `You are ZYROX AI Coach — an elite personal trainer and sports nutritionist integrated into the 3D Body Scanner fitness app.
...
function buildPlannerPrompt(locale: AILocale): string {
  const lang = LANGUAGE_NAME[locale] ?? LANGUAGE_NAME.pt;
  return `You are an expert periodization and sports nutrition system integrated into ZYROX 3D Body Scanner.
