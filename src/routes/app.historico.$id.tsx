...
export const Route = createFileRoute("/app/historico/$id")({
  head: ({ params }) => ({
    meta: [{ title: `${getHistoryEntry(params.id)?.name ?? "Histórico"} · ZYROX` }],
  }),
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-border bg-surface p-5">
        ...
        <h1 className="mt-1 font-display text-2xl font-bold">{entry.name}</h1>
          ...
          <div key={i} className="rounded-2xl border border-border bg-surface p-4">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-muted-foreground">{ex.muscle} · {ex.sets.length} séries</div>
              </div>
