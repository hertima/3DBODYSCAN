...
const COACH_MSG: Record<string, (name: string, count: number, duration: number, exercises: string) => string> = {
  pt: (name, count, duration, exercises) => `Vou fazer o ${name} agora (${count} exercícios, ${duration} min). Exercícios principais: ${exercises}. Quais dicas você me dá para maximizar os resultados hoje?`,
  es: (name, count, duration, exercises) => `Voy a hacer ${name} ahora (${count} ejercicios, ${duration} min). Ejercicios principales: ${exercises}. ¿Qué consejos me das para maximizar los resultados hoy?`,
  en: (name, count, duration, exercises) => `I'm about to do ${name} (${count} exercises, ${duration} min). Main exercises: ${exercises}. What tips do you have to maximize my results today?`,
  fr: (name, count, duration, exercises) => `Je vais faire ${name} maintenant (${count} exercices, ${duration} min). Exercices principaux : ${exercises}. Quels conseils me donnez-vous pour maximiser mes résultats aujourd'hui ?`,
  de: (name, count, duration, exercises) => `Ich mache jetzt ${name} (${count} Übungen, ${duration} Min). Hauptübungen: ${exercises}. Welche Tipps hast du, um meine heutigen Ergebnisse zu maximieren?`,
};
          ...
          <button
            ...
            onClick={() => {
              const exerciseNames = workout.exercises
                ...
                .filter(Boolean)
                .join(", ");
              const msg = (COACH_MSG[locale] ?? COACH_MSG.pt)(workout.name, workout.exercises.length, workout.duration, exerciseNames);
                    ...
                    return (
                      ...
                      >
                        <div className="h-12 w-12 shrink-0 overflow-hidden rounded-xl border border-border bg-elevated">
                          <ExerciseMedia exerciseId={alt.id} src={alt.gifPath ?? undefined} size="thumb" className="h-full w-full" />
                        </div>
